# Set Project Name
PROJECT_NAME = freepad_tlsr_zed
PROJECT_MODEL ?= _model_1

# Set the serial port number for downloading the firmware
DOWNLOAD_PORT := COM3

ifeq ($(PROJECT_MODEL),_model_1)
	PROJECT_DEF = "-DDEVICE_MODEL=DEVICE_BUTTON_8"
	PROJECT_BUTTON = _08K
else
	ifeq ($(PROJECT_MODEL),_model_2)
		PROJECT_DEF = "-DDEVICE_MODEL=DEVICE_BUTTON_12"
		PROJECT_BUTTON = _12K
	else
		ifeq ($(PROJECT_MODEL),_model_3)
			PROJECT_DEF = "-DDEVICE_MODEL=DEVICE_BUTTON_20"
			PROJECT_BUTTON = _20K
		else
			PROJECT_DEF = "-DDEVICE_MODEL=DEVICE_BUTTON_8"
			PROJECT_BUTTON = _08K
...
PROJECT_FILE_NAME := $(PROJECT_NAME)$(PROJECT_BUTTON)
...
flash-orig-read-1024:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m rf 0 0x100000 freepad_tlsr_zed_orig.bin
